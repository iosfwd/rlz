...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        ...
        // use the parsing file generated by the parser program (now in binary) 
        // instead of computing the spl vector from scratch from ref-, sa-, input-files
        std::fprintf(stderr, "usage: %s [reference file] [parsing file] [input file]\n", argv[0]);
        std::exit(1);
    }
    ...
    std::vector<std::tuple<std::size_t, std::size_t, std::size_t>> res;
